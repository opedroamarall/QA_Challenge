...
export class OperationsPage {
    ...
    async fillPracticeForm(firstName: string, lastName: string, email: string): Promise<void> {
    ...
    async createWebTableRecord(fName: string, lName: string, email: string, age: string, salary: string, dept: string): Promise<void> {
    ...
    async createMultipleRecords(count: number): Promise<void> {
        ...
        for (let i = 1; i <= count; i++) {
            await this.createWebTableRecord(`User${i}`, `Test${i}`, `dynamic${i}@test.com`, '30', '2000', 'IT');
        }
    }
    ...
    async handleProgressBar(): Promise<void> {
        ...
        await this.page.waitForFunction(() => {
            const val = parseInt(document.querySelector('div[role="progressbar"]')?.getAttribute('aria-valuenow') || '0');
        ...
        // Validation
        const valueAfterStop = parseInt(await progressBar.getAttribute('aria-valuenow') || '0');
        ...
        await expect(progressBar).toHaveAttribute('aria-valuenow', '0', { timeout: 10000 });
    }
   ...
   async sortListDescending(): Promise<void> {
        ...
        for (let i = 0; i < targetOrder.length; i++) {
            ...
            const itemToMove = this.page.locator('.list-group-item').filter({ hasText: itemName }).first();
        ...
        const listItems = this.page.locator('.vertical-list-container .list-group-item');
